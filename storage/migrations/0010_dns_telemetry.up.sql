BEGIN;
CREATE TABLE platform.dns_batches(
...
CREATE TABLE platform.dns_events(
 ...
 transaction_entity_id text, event_type text NOT NULL CHECK(event_type IN
 ('query_observed','response_observed','query_failed','transaction_observed')),
 ...
 native_event_id text, raw_sha256 text, policy_version text,
 original_query_name text NOT NULL, canonical_query_name text NOT NULL,
 record_type text, response_code text, resolver_address inet, process_entity_id text,
 resolved_addresses inet[] NOT NULL DEFAULT '{}', data_quality_flags text[] NOT NULL DEFAULT '{}',
 late boolean NOT NULL DEFAULT false, out_of_order boolean NOT NULL DEFAULT false,
 event_data jsonb NOT NULL, PRIMARY KEY(tenant_id,event_id),
 UNIQUE(tenant_id,endpoint_id,agent_id,sequence),
 FOREIGN KEY(tenant_id,batch_id) REFERENCES platform.dns_batches(tenant_id,batch_id));
CREATE INDEX dns_events_timeline ON platform.dns_events(tenant_id,endpoint_id,observed_at DESC,event_id DESC);
CREATE INDEX dns_events_name ON platform.dns_events(tenant_id,canonical_query_name,observed_at DESC);
CREATE INDEX dns_events_transaction ON platform.dns_events(tenant_id,endpoint_id,transaction_entity_id,observed_at,event_id);
CREATE INDEX dns_events_process ON platform.dns_events(tenant_id,endpoint_id,process_entity_id,observed_at DESC) WHERE process_entity_id IS NOT NULL;
CREATE INDEX dns_events_answer ON platform.dns_events USING gin(resolved_addresses);

CREATE TABLE platform.dns_transactions(
 tenant_id uuid NOT NULL REFERENCES platform.tenants(id), endpoint_id uuid NOT NULL,
 transaction_entity_id text NOT NULL, canonical_query_name text NOT NULL, record_type text,
 first_observed timestamptz NOT NULL, last_observed timestamptz NOT NULL,
 query_event_id uuid, response_event_id uuid, state text NOT NULL,
 transaction_confidence text NOT NULL, process_entity_id text, latest_event jsonb NOT NULL,
 PRIMARY KEY(tenant_id,endpoint_id,transaction_entity_id));
CREATE TABLE platform.dns_answer_records(
 tenant_id uuid NOT NULL REFERENCES platform.tenants(id), event_id uuid NOT NULL,
 ordinal integer NOT NULL, answer_data jsonb NOT NULL,
 PRIMARY KEY(tenant_id,event_id,ordinal),
 FOREIGN KEY(tenant_id,event_id) REFERENCES platform.dns_events(tenant_id,event_id) ON DELETE CASCADE);
CREATE TABLE platform.dns_network_relationships(
 tenant_id uuid NOT NULL REFERENCES platform.tenants(id), dns_event_id uuid NOT NULL,
 network_event_id uuid NOT NULL, relationship_data jsonb NOT NULL,
 PRIMARY KEY(tenant_id,dns_event_id,network_event_id));
CREATE TABLE platform.dns_telemetry_health(
 tenant_id uuid NOT NULL REFERENCES platform.tenants(id), endpoint_id uuid NOT NULL,
 health_data jsonb NOT NULL, updated_at timestamptz NOT NULL DEFAULT now(),
 PRIMARY KEY(tenant_id,endpoint_id));
CREATE TABLE platform.dns_policy_versions(
...
CREATE TABLE platform.dns_policy_assignments(
 tenant_id uuid NOT NULL REFERENCES platform.tenants(id), endpoint_id uuid,
 policy_id uuid NOT NULL REFERENCES platform.dns_policy_versions(id), assigned_at timestamptz NOT NULL DEFAULT now(),
 assigned_by text NOT NULL, UNIQUE NULLS NOT DISTINCT(tenant_id,endpoint_id));
CREATE TABLE platform.dns_policy_acknowledgements(
 ...
 PRIMARY KEY(tenant_id,endpoint_id));
CREATE TABLE platform.dns_export_jobs(
...
CREATE INDEX dns_export_pending ON platform.dns_export_jobs(created_at) WHERE state='pending';
COMMIT;
